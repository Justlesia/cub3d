...
/*   Updated: 2021/02/10 17:19:43 by sbrenton         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_resolution ft_resolution(char *line)
{
	...
	line = ft_mystrim(line);
	r.x =  ft_atoi(line);
	line = ft_mystrim(&(line[ft_strlen(ft_itoa(r.x))]));
	r.y =  ft_atoi(line);
	line = ft_mystrim(&(line[ft_strlen(ft_itoa(r.y))]));
...
//		free(line);
	...
	if (prm->player.r == -1)
		return NULL;
	prm->p_cam = ft_camera_params(prm->player.view,prm->p_cam);
	return prm;
}
